...
module INNER-SYNTAX
  ...
  // inner expressions, produce computations results
...
module EXC-SYNTAX
  ...
  // outer expressions, reduced to inner expressions
  syntax Expr ::= Result
                | Value
                | Var
                > "(" Expr ")"            [bracket]
  ...
  // if / else / while - must be followed by explicit new blocks
  syntax Block ::= "{" "}"
                | "{" Stmt "}"
...
module SIMPLIFY
  ...
  rule simplify(A:Float + B:Int) => A +Float #toFloat(B)
  rule simplify(A:Int + B:Float) => #toFloat(A) +Float B
  rule simplify(A:Float - B:Int) => A -Float #toFloat(B)
  rule simplify(A:Int - B:Float) => #toFloat(A) -Float B

  rule simplify(A:Float + B:Float) => A +Float B
  rule simplify(A:Float + 0.0) => A
  rule simplify(A:Float + -0.0) => A
  rule simplify(0.0 + B:Float) => B
  rule simplify(-0.0 + B:Float) => B
  rule simplify(A:Float - B:Float) => A -Float B
  rule simplify(A:Float - 0.0) => A
  rule simplify(A:Float - -0.0) => A
...
module EXC
  ...
  rule <k> float V:Var = (int, Val) ; => . ...</k>
       <vars> Vs => Vs [V <- (float, #toFloat(Val))] </vars>
    requires notBool(V in_keys(Vs))

  rule <k> T1:Type V:Var = (T2, _) ; #as S => AssignmentTypingError(S, T1, T2)  ...</k>
       <vars> ... V |-> _ ... </vars>
    [owise]
  ...
  rule <k> V:Var = (int, Val) ; => . ...</k>
       <vars>... V |-> (float, (_ => #toFloat(Val))) </vars>
  ...
  // type conversions
  rule V:Int => (int, V)
  rule V:Float => (float, V)
  rule V:Bool => (bool, V)

  // inner arithmetic expressions
  rule (T1:Type, V1) + (T2:Type, V2) => (#leastCommonType(T1, T2), simplify(V1 + V2))
    requires #isNumericType(T1) andBool #isNumericType(T2)
  rule (T1:Type, V1) - (T2:Type, V2) => (#leastCommonType(T1, T2), simplify(V1 - V2))
    requires #isNumericType(T1) andBool #isNumericType(T2)
  rule (T1:Type, V1) * (T2:Type, V2) => (#leastCommonType(T1, T2), simplify(V1 * V2))
    requires #isNumericType(T1) andBool #isNumericType(T2)
  ...
  rule (T:Type, V1) != (T, V2) => (bool, simplify(V1 != V2))
  rule (T:Type, V1) < (T, V2) => (bool, simplify(V1 < V2))
    requires #isNumericType(T)
  rule (T:Type, V1) >= (T, V2) => (bool, simplify(V1 >= V2))
    requires #isNumericType(T)
  rule (T:Type, V1) > (T, V2) => (bool, simplify(V2 < V1))
    requires #isNumericType(T)
  rule (T:Type, V1) <= (T, V2) => (bool, simplify(V2 >= V1))
    requires #isNumericType(T)
  ...
  rule while ( (int, _) ) _ #as S => IfTypingError(S, bool, int)
    [owise]
  rule while ( (float, _) ) _ #as S => IfTypingError(S, bool, float)
    [owise]
endmodule
```
